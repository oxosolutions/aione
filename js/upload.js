jQuery(document).ready(function($){
	if($('.aione_upload_button').length ) {
		window.aione_uploadfield = '';

		$('.aione_upload_button').live('click', function() {
	...
	if($.cookie('aione_metabox_tab_' + jQuery('#post_ID').val())) {
		var id = $.cookie('aione_metabox_tab_' + jQuery('#post_ID').val());
	...
	jQuery('.pyre_metabox_tabs li a').click(function(e) {
		...
		$.cookie('aione_metabox_tab_' + jQuery('#post_ID').val(), id, { expires: 7 });
...
function calc_element_heights() {
	...
	jQuery( '.pyre_field .aione-shortcodes-arrow' ).each( function() {
	...
	jQuery( '.pyre_field .aione_upload_button' ).each( function() {
